...
public interface StartupStakeholderRepository extends JpaRepository<StartupStakeholder, Long> {
    ...
    @Query("SELECT new com.startupsphere.capstone.dto.StakeholderInfoDTO(ss.stakeholder, ss.role, ss.status, ss.dateJoined) FROM StartupStakeholder ss WHERE ss.startup.id = :startupId")
    List<com.startupsphere.capstone.dto.StakeholderInfoDTO> findStakeholderInfoByStartupId(@Param("startupId") Long startupId);
}